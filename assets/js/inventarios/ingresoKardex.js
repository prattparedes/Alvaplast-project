...
async function obtenerDatosOCKardex(idCompra) {
  ...
  let id = parseInt(idCompra);
  // URLs dinámicas basadas en los valores extraídos
  const urlCompra = `http://localhost/Alvaplast-project/Controller/compras/CompraController.php?idCompra=${id}`;
  const urlCompraProducto = `http://localhost/Alvaplast-project/Controller/compras/CompraProductoController.php?idCompra=${id}`;
...
async function listarProductoxMarca() {
  const marca = document.getElementById("marca_select").value
  if (marca == "0" || !marca) {
    return alert("seleccionar una marca  por favor")
  }
  const url = `http://localhost/Alvaplast-project/Controller/maintenance_models/MarcaController.php?marca=${marca}`;
  ...
  const dataMarca = await response.json();
