    ...
    if (xhr.readyState === XMLHttpRequest.DONE) {
      if (xhr.status === 200) {
        ...
        if (xhr.responseText) {
          ...
          // Obtener la referencia del cuerpo de la tabla
          const tbody = document.getElementById("ventaxCosto");
          ...
          labelSumaUtilidadesFechaInicio.textContent = "Suma de Utilidades (Fecha de Inicio): " + (sumasPorDia[formattedFecha1] || 0);
          ...
          labelSumaUtilidadesFechaFin.textContent = "Suma de Utilidades (Fecha de Fin): " + (sumasPorDia[formattedFecha2] || 0);
        } else {
          // Hubo un error en la solicitud
          console.error("Error en la solicitud.");
        }
...
function exportarCostoPDF() {
  ...
  Object.keys(sumasPorDia).forEach(function (fecha) {
    ...
    doc.setFillColor(255, 0, 0); // Relleno rojo
